...
Device::DPurityMonitor *
configurePurityMonitor (const Configuration::PurityMonitor & config,
			AddressSpace::ASNodeManager * nm,
			UaNodeId parentNodeId,
			Device::Parent_DPurityMonitor * parentDevice);
		       ...
		       Device::Parent_DAirFlowProbe * parentDevice)
{
  ...
  return dItem;
	...
	//! Called to create every single instance of PurityMonitor 
Device::DPurityMonitor *
configurePurityMonitor (const Configuration::PurityMonitor & config,
			AddressSpace::ASNodeManager * nm,
			UaNodeId parentNodeId,
			Device::Parent_DPurityMonitor * parentDevice)
{
  AddressSpace::ASPurityMonitor * asItem =
    new AddressSpace::ASPurityMonitor (parentNodeId,
				       nm->
				       getTypeNodeId (AddressSpace::
						      ASInformationModel::AS_TYPE_PURITYMONITOR),
				       nm, config);


  Device::DPurityMonitor * dItem = new Device::DPurityMonitor (config,
							       parentDevice);
  asItem->linkDevice (dItem);
  dItem->linkAddressSpace (asItem, asItem->nodeId ().toString ().toUtf8 ());
	   ...
	   ConfigXmlDecoratorFunction configXmlDecoratorFunction)
{
  ...
  for (size_t i = 0; i < childrenNodes->getLength (); ++i)
      ...
      if (tagName == "Controller")
	{
      ...
      if (tagName == "PurityMonitor")
	{
	  auto it = std::find_if (config.PurityMonitor ().begin (),
				  config.PurityMonitor ().end (),
				  [element] (const Configuration::
					     PurityMonitor & x) { return x.
				  _node () == element;
				  }
	  );


	  Device::DPurityMonitor * newObject =
	    configurePurityMonitor (*it, nm, rootNode, deviceRoot);
...
unlinkAllDevices (AddressSpace::ASNodeManager * nm)
{
  ...
  {
    std::vector < AddressSpace::ASPurityMonitor * >objects;
    std::string pattern (".*");
    AddressSpace::findAllObjectsByPatternInNodeManager <
      AddressSpace::ASPurityMonitor > (nm, pattern, objects);
    totalObjectsNumber += objects.size ();
  for (AddressSpace::ASPurityMonitor * a:objects)
      {
	a->unlinkDevice ();
      }
  }
