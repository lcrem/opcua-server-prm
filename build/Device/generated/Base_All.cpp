...
Base_DAirFlowProbe::Base_DAirFlowProbe (const Configuration::AirFlowProbe & config, Parent_DAirFlowProbe * parent):
  ...
  void Base_DPurityMonitor::linkAddressSpace (AddressSpace::ASPurityMonitor *
					      addressSpaceLink,
					      const std::
					      string & stringAddress)
  {
  ...
  AddressSpace::ASPurityMonitor * Base_DPurityMonitor::getAddressSpaceLink ()const
  {
    if (m_addressSpaceLink)
      return m_addressSpaceLink;
    else
  ...
  //! Disconnects AddressSpace part from the Device logic, and does the same for all children
  //! Returns number of unlinked objects including self
  unsigned int Base_DPurityMonitor::unlinkAllChildren ()
  {
    unsigned int objectCounter = 1;	// 1 is for self
    m_addressSpaceLink = 0;
    /* Fill up: call unlinkAllChildren on all children */
  ...
  /* Constructor */
Base_DPurityMonitor::Base_DPurityMonitor (const Configuration::PurityMonitor & config, Parent_DPurityMonitor * parent):
  ...
  Base_DPurityMonitor::~Base_DPurityMonitor ()
  {
    /* remove children */
  ...
  std::list < DPurityMonitor * >Base_DPurityMonitor::s_orphanedObjects;
